...
# ---------------- Header ----------------
st.title("🧭 PERSONAL 360 — V4")
st.markdown(
    '<div class="hero"><b>Od finansijske baze do kompetencije:</b> '
    'finansije → prioriteti → ciljevi → vrijednosti → obrasci → kompetencije.</div>',
    unsafe_allow_html=True,
)
st.caption(
...
with tabs[5]:
    ...
    st.caption("BITNO/HITNO · BITNO/NIJE HITNO · NIJE BITNO/HITNO · NIJE BITNO/NIJE HITNO. Polja Cilj i Vrijednost su pripremljena za buduće module.")
    ...
    with st.form("eisenhower_form", clear_on_submit=True):
        ...
        c6, c7 = st.columns(2)
        goal_link = c6.text_input("Cilj (opciono — budući modul)")
        value_link = c7.text_input("Vrijednost (opciono — budući modul)")
    ...
    st.write("V4 je pripremljen da se zadaci kasnije povežu sa **ličnim vrijednostima, ciljevima, obrascima/trigerima i zajedničkim Personal 360 dashboardom**.")
...
with tabs[6]:
    ...
    st.download_button(
        ...
        data=xlsx,
        file_name="Personal_360_V4_snapshot.xlsx",
